...
func (e *Engine) build(ctx context.Context, dc *Context, opt Options) error {
	...
	case deployspec.KindStatic, deployspec.KindFramework:
		if err := e.buildGenerated(ctx, dc, tag); err != nil {
			return err
		}
	default:
...
// buildGenerated builds a project publix wrote the Dockerfile for.
//
// The generated Dockerfile is injected into the build context rather than
// written to the checkout: publix never modifies a repository, and a user
// who wants control commits their own Dockerfile, which detection then
// defers to.
func (e *Engine) buildGenerated(ctx context.Context, dc *Context, tag string) error {
	sp := dc.Spec
	result, err := e.renderFor(dc)
	if err != nil {
		return err
	}

	dockerfile, err := result.Render()
	if err != nil {
		return err
	}

	files := map[string][]byte{"Dockerfile.publix": dockerfile}
	if result.Mode == framework.ModeStatic {
		files["nginx.publix.conf"] = result.NginxConf()
	}

	dc.Log.Printf("Building %s with a generated Dockerfile for %s", tag, result.Name)
	// Echo the generated file into the log. It is not in the repository,
	// so this is the only place a user can see what actually got built —
	// and the first thing they need when a generated build goes wrong.
	for _, line := range strings.Split(strings.TrimRight(string(dockerfile), "\n"), "\n") {
		dc.Log.Printf("  | %s", line)
	}

	out := dc.Log.Writer(buildlog.StreamStdout)
	defer out.Flush()

	if err := e.docker.BuildImage(ctx, dockerapi.BuildOptions{
		Context:    filepath.Join(dc.Root, sp.Context),
...
// renderFor produces the framework result a deployment should build from:
// what was detected in the checkout, with the spec's overrides applied.
func (e *Engine) renderFor(dc *Context) (*framework.Result, error) {
	sp := dc.Spec
	result := dc.Spec.Detection.Result()
	if result == nil {
		result = framework.Detect(framework.Dir(dc.Root))
	}

	// A spec naming a framework explicitly overrules detection, which
	// matters for a repository whose config publix reads wrongly.
	if sp.Framework != "" && sp.Framework != result.ID {
		dc.Log.Printf("deployment.yaml pins the %s template (detected %s)", sp.Framework, result.ID)
	}

	result = result.Apply(framework.Overrides{
		Install: sp.Build.Install,
		Build:   sp.Build.Command,
		Start:   sp.Build.Start,
		Output:  sp.Build.Output,
		Builder: sp.Build.Builder,
		Runtime: sp.Build.Runtime,
		Port:    sp.Port,
		SPA:     sp.Build.SPA,
	})

	// The spec's own kind is authoritative: a project told to be static is
	// static even if its config file suggests otherwise.
	switch sp.Kind {
	case deployspec.KindStatic:
		result.Mode = framework.ModeStatic
	case deployspec.KindFramework:
		if result.Mode == framework.ModeUnknown {
			return nil, fmt.Errorf("publix could not work out how to build this project; set build.start and port in deployment.yaml")
		}
	}
	return result, nil
}
